...
def disp_delay(freq1=0.184975, freq2=0.200335, DM=0):
	"""Accepts two frequencies as input (Low then High in GHz) as well as a DM and reports the delay in ms"""
...
def E_rot(p=1.0, *args):
	"""Accepts a spin period (in s) and returns the approximate roational energy in J. Also accepts an optional value for the moment of inertia I, else reverts to the standard estimate."""
...
def E_loss(p=1.0, pdot=1e-12, *args):
	"""Accepts a spin period (in s) and spin period derivative (in s/s) and returns the approximate roational energy loss rate in J/s. Also accepts optional value for moment of inertia I, else reverts to standard estimate."""
...
def min_mag_field(p=1.0, pdot=1.0e-12):
	"""Accepts a spin period (in s) and spin period derivative (in s/s) and returns the minimum surface magnetic field strength in Gauss."""
...
def char_age(p=1.0, pdot=1e-12):
	"""Accepts a spin period (in s) and spin period derivative (in s/s) and returns the characteristic age (in years)"""
